...
/**
 * Implementation of Dekker's Algorithm using the sequential consistency
 * memory order model. Dekker's Algorithm is correct under sequential
 * consistency.
 */
...
void *thread1_run()
{
     ...
     while (true) {
	  ...
	  while (want2.load(std::memory_order_seq_cst)) {
	       if (turn.load(std::memory_order_seq_cst) != 0) {
		    want1.store(false, std::memory_order_seq_cst);
...
void *thread2_run()
{
     ...
     while (true) {
	  // Pre-protocol
	  want2.store(true, std::memory_order_seq_cst);
	  while (want1.load(std::memory_order_seq_cst)) {
	       if (turn.load(std::memory_order_seq_cst) != 1) {
		    want2.store(false, std::memory_order_seq_cst);
