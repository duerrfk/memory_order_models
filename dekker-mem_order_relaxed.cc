...
/**
 * Implementation of Dekker's Algorithm using the relaxed memory order.
 * The purpose of this implementation is to practically show whether 
 * Dekker's algorithm is guaranteeing mutual exclusion with relaxed memory 
 * order.
 */
...
void *thread1_run()
{
     ...
     while (true) {
	  // Pre-protocol
	  want1.store(true, std::memory_order_relaxed);
	  while (want2.load(std::memory_order_relaxed)) {
	       if (turn.load(std::memory_order_relaxed) != 0) {
		    want1.store(false, std::memory_order_relaxed);
...
void *thread2_run()
{
     ...
     while (true) {
	  // Pre-protocol
	  want2.store(true, std::memory_order_relaxed);
	  while (want1.load(std::memory_order_relaxed)) {
	       if (turn.load(std::memory_order_relaxed) != 1) {
		    want2.store(false, std::memory_order_relaxed);
